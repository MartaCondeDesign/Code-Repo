...
41. Prohibición de Registros de Paquetes y Dominios Excluidos como Documentación Oficial (npmjs.com, fast.design): Los enlaces a dominios excluidos como `fast.design` (`https://fast.design/`), registros de paquetes como `npmjs.com` (ejemplo `https://www.npmjs.com/package/@primer/react`), `npmjs.org`, `yarnpkg.com`, `pypi.org`, `crates.io`, `rubygems.org` o similares NUNCA pueden ser clasificados como sitios de documentación oficial. No pueden devolverse como `OFFICIAL_DOCUMENTATION` ni aparecer en las tarjetas de métricas o enlaces de documentación de Project Guide.
42. Regla Visual Estricta de Divisores y Espaciados en el Panel Inspector Derecho: Prohibición absoluta de divisores dobles o consecutivos sin contenido entre ellos. Los divisores sirven exclusivamente para separar bloques de información; en ningún caso pueden figurar dos líneas divisorias seguidas. El espaciado entre cualquier línea divisoria y el contenido adyacente debe ser de exactamente **24px**. Dentro de una misma sección, la separación entre el texto descriptivo (como "QUÉ ES") y los bloques de contenido es de **16px**, mientras que la separación entre bloques adyacentes (como cuadrículas de detalle y el visor de código `.code-shell`) es de **8px**.
